...
var _ = Describe("Consumer", func() {
	...
	It("should consume/commit/resume", func() {
		...
		go consume("M04", 200)
		go consume("M05", 100)
		Eventually(func() int { return len(acc) }, "30s", "50ms").Should(BeNumerically(">=", 3000))

		go consume("M06", 300)
		go consume("M07", 400)
		go consume("M08", 500)
		go consume("M09", 2000)
		Eventually(func() int { return len(acc) }, "30s", "50ms").Should(BeNumerically(">=", 8000))

		go consume("M10", 1000)
		Eventually(func() int { return len(acc) }, "30s", "50ms").Should(BeNumerically(">=", 10000))

		go consume("M11", 1000)
		go consume("M12", 2500)
		Eventually(func() int { return len(acc) }, "30s", "50ms").Should(BeNumerically(">=", 12000))

		go consume("M13", 1000)
		Eventually(func() int { return len(acc) }, "30s", "50ms").Should(BeNumerically(">=", 15000))
